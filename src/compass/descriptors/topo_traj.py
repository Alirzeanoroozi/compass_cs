...
def prepare_datastructures(trajectories, topo, out_dir, heavies, first_timer):
    ...
    return resids_to_atoms, resids_to_noh, atoms_to_resids, calphas, oxy, nitro, donors, hydros, acceptors
...
def get_resids_indices(trajectory):
    ...
    for i, resid in enumerate(backbone_res_indices):
        ...
        res_ind_zero[i] = np.asarray([atom.index for atom in residue.atoms], dtype=np.int32)
        res_ind_noh[i] = np.asarray([atom.index for atom in residue.atoms if atom.element is None or atom.element.symbol != "H"], dtype=np.int32)
...
def get_sb_indices(topo_df, atoms_to_resids):
    # {residue index: [oxygen indices], [nitrogen indices]}
